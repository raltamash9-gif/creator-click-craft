            ...
            {steps.map((s, i) => (
              <Reveal key={s.n} delay={Math.min(i * 0.05, 0.3)}>
                <li className="group relative grid grid-cols-[auto_minmax(0,1fr)] gap-6 lg:gap-10">
                  <span className="relative z-10 mt-1 flex h-7 w-7 items-center justify-center rounded-full border border-border bg-background text-[10px] font-semibold tracking-[0.05em] text-subtle transition-all duration-500 group-hover:border-accent group-hover:bg-accent group-hover:text-accent-foreground lg:h-8 lg:w-8 lg:text-[11px]">
                    {s.n}
                  </span>
                  <div
                    data-cursor="link"
                  ...
                  >
                    <h3 className="text-[clamp(1.25rem,2vw,1.75rem)] font-bold tracking-[0.01em]">{s.title}</h3>
